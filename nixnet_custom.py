...
class nixnet:
    ...
    def __init__(self, intf_type = 'CAN'):
        ...
        #self.XNET_dll = cdll.LoadLibrary(r"./usr/local/natinst/lib/libnixnet.so.19.6.0")
        self.XNET_dll = cdll.LoadLibrary(r"libnixnet.so")
    ...
    def create_frame_input_session(self, intf_port, intf_database = ":memory:", intf_cluster = '', intf_frames = ''):
        ...
        try:
            ...
            if status < 0:
                raise Exception('XNET Session Error in create_frame_input:  {}'.format(status))
